...
export function TaskCard({ task, onChecklistToggle }: TaskCardProps) {
  const typeColors = {
    PROA: 'bg-indigo-50 text-indigo-700 border-indigo-200',
  ...
  return (
    <div
      className={`bg-white rounded-lg border ${statusColors[task.status]} p-4 shadow-sm hover:shadow-md transition-shadow duration-200`}
    >
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <p className="text-xs font-medium text-slate-500 mb-1">{task.institution}</p>
          <h3 className="text-sm font-semibold text-slate-800 leading-tight">{task.title}</h3>
        </div>
        <span
          className={`px-2.5 py-1 rounded-md text-xs font-semibold border ${typeColors[task.type]}`}
        >
          {task.type}
        </span>
      </div>
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-500 mt-1.5">Progreso: {task.progress}%</p>
      </div>
